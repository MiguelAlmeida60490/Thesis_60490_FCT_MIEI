...
Ao contrário dos modelos anteriores de \textit{deep learning} em que é necessário algum tipo de calibração ou ajustes específicos para cada tarefa, os (\texorpdfstring{\gls{LLMs}}{LLMs}) modernos não necessitam de tanta informação ou exemplos para conseguir devolver uma resposta. Isto significa que podem realizar novas tarefas com base apenas em instruções de linguagem natural, sem necessidade de treino adicional extensivo~\cite{brown2020gpt3}. Esta característica é fundamental para a sua aplicação prática em ambientes empresariais, onde a diversidade de tarefas e a necessidade de rápida adaptação são elevadas.
