...
Ao contrário de abordagens reativas baseadas em eventos da plataforma de controlo de versões, a solução proposta assenta num modelo proativo, no qual a análise das alterações de código é desencadeada antes da criação de uma \gls{PR}. Esta decisão permite alinhar a solução com as práticas existentes na Processware e reforçar o papel da inteligência artificial como mecanismo de apoio à qualidade do código desde fases iniciais do desenvolvimento. Esta diferença conceptual entre o fluxo atualmente utilizado e o fluxo proposto é ilustrada na Figura~\ref{fig:fluxo_code_review}, onde se evidencia a antecipação da análise automática para uma fase anterior à criação da \gls{PR}.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{5-Figures/Thesis_Images/fluxo_code_review}
    \caption{Comparação entre o fluxo atual de revisão de código e o fluxo de revisão de código da solução proposta}
    \label{fig:fluxo_code_review}
\end{figure}
...
O fluxo completo de execução do processo de análise automática das alterações de código, desde a solicitação inicial pelo programador até à apresentação dos comentários de revisão no O2P, encontra-se representado na Figura~\ref{fig:fluxo_AI}.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{5-Figures/Thesis_Images/fluxo_AI}
    \caption{Fluxo de execução do processo de análise automática das alterações de código antes da criação de uma \gls{PR}}
    \label{fig:fluxo_AI}
\end{figure}
...
Os requisitos funcionais definem as capacidades que o sistema deve oferecer para apoiar o processo de revisão de código. Estes requisitos cobrem todo o fluxo de utilização da solução, desde a seleção dos ramos a comparar até à apresentação dos comentários de revisão no O2P, assegurando que o sistema atua como um mecanismo de apoio contínuo ao programador antes da criação da \gls{PR}. A Tabela~\ref{tab:requisitos_funcionais} apresenta de forma estruturada os requisitos funcionais identificados para a solução proposta.

\begin{table}[h]
\centering
\caption{Requisitos funcionais da solução proposta}
\label{tab:requisitos_funcionais}
\begin{tabular}{|c|p{12cm}|}
\hline
\textbf{ID} & \textbf{Descrição} \\ \hline
RF1 & Seleção da \textit{branch} de trabalho e da \textit{branch} base para análise. \\ \hline
RF2 & Extração automática das diferenças entre os ramos selecionados, incluindo o \gls{Diff} e os ficheiros modificados. \\ \hline
RF3 & Pré-processamento das alterações de código, incluindo filtragem e organização da informação relevante. \\ \hline
RF4 & Análise automática das alterações utilizando \glspl{LLM} para identificação de problemas e oportunidades de melhoria. \\ \hline
RF5 & Integração explícita de boas práticas internas da Processware no processo de análise. \\ \hline
RF6 & Geração de comentários de revisão claros, concisos e em linguagem natural. \\ \hline
RF7 & Apresentação dos comentários gerados no O2P antes da criação da \gls{PR}. \\ \hline
\end{tabular}
\end{table}
...
A solução deve igualmente cumprir um conjunto de requisitos não funcionais que garantem a sua adequação ao contexto organizacional da Processware. Estes requisitos refletem preocupações relacionadas com a integração com sistemas internos, desempenho, escalabilidade e capacidade de evolução futura da solução. A Tabela~\ref{tab:requisitos_nao_funcionais} sintetiza os requisitos não funcionais definidos para a solução proposta.

\begin{table}[h]
\centering
\caption{Requisitos não funcionais da solução proposta}
\label{tab:requisitos_nao_funcionais}
\begin{tabular}{|c|p{12cm}|}
\hline
\textbf{ID} & \textbf{Descrição} \\ \hline
RNF1 & Integração nativa com os sistemas internos da Processware, em particular o O2P. \\ \hline
RNF2 & Baixo acoplamento a plataformas externas de controlo de versões, como o Bitbucket. \\ \hline
RNF3 & Capacidade de suportar múltiplas análises em paralelo, garantindo escalabilidade. \\ \hline
RNF4 & Tempo de resposta compatível com o fluxo de trabalho do programador. \\ \hline
RNF5 & Arquitetura orientada à manutenibilidade e extensibilidade futura da solução. \\ \hline
\end{tabular}
\end{table}
...
O funcionamento do sistema pressupõe um conjunto de condições de utilização que enquadram a sua aplicação no contexto organizacional da Processware e asseguram a correta integração da solução no fluxo de desenvolvimento existente. A Tabela~\ref{tab:condicoes_utilizacao} apresenta de forma estruturada as condições de utilização consideradas.

\begin{table}[h]
\centering
\caption{Condições de utilização da solução}
\label{tab:condicoes_utilizacao}
\begin{tabular}{|c|p{12cm}|}
\hline
\textbf{ID} & \textbf{Descrição} \\ \hline
CU1 & Utilização de um sistema de controlo de versões baseado em ramos. \\ \hline
CU2 & Existência de boas práticas internas documentadas e acessíveis. \\ \hline
CU3 & Supervisão humana das recomendações geradas automaticamente pelo sistema. \\ \hline
CU4 & Utilização do O2P como ponto central do fluxo de desenvolvimento. \\ \hline
\end{tabular}
\end{table}
